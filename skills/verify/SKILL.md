...
4. **Red → classify, then fix.** New-code failure: 3 attempts. Regression
   ...
   Environment or tooling: 1, then report. Budgets count per failure
   across implement + verify — a failure carried in keeps its remaining
   attempts. Fixes land against the feat commit: `--amend` while it is
   HEAD; once refactor commits sit on top, a `fix(scope) [C-id]` commit —
   never rewrite buried history. Budget spent → stop, attempts +
   hypothesis to ADR.md, human.
...
6. **Tick** — only now: the C-id's box in SPEC.md, the task's box in
   PLAN.md.
7. **Route** — on green: every batch task ticked → "→ review", otherwise
   "→ implement, task N". Budget spent → the human has the floor; no
   route.
...
- Land a fix after a box is ticked — evidence and commit precede the tick.
